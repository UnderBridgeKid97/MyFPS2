    ...
    public class PlayerInputHandler : MonoBehaviour
    {
        ...
        public int GetSwitchWeaponInput()
        {
            if(CanProcessInput())
            {
                ...
                else if (Input.GetAxis(GameConstants.k_AxisNameNextWeapon) < 0f)     // 위 두개는 q, e
                {
                    return -1;
                }
                ...
                else if (Input.GetAxis(GameConstants.k_MouseAxisNameScrollWheel) < 0f)  // 아래두개는 마우스 휠
                {
                    return -1;
                }
            }
        ...
        // 조준 -  마우스 우클릭하는 동안 & left alt
        public bool GetAimInputHeld()
        {
            if (CanProcessInput()) // 커서에 락 걸려잇을 때만
            {
